...
class SecondCurrencyRecyclerAdapter  @Inject constructor (
    @ActivityContext private val context: Context
) : RecyclerView.Adapter<SecondCurrencyRecyclerAdapter.ViewHolder>() {
    private var currencyDateList = ArrayList<String>()
    private var currencyList = ArrayList<String>()
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): SecondCurrencyRecyclerAdapter.ViewHolder {
        return ViewHolder(
            LayoutInflater.from(context).inflate(R.layout.currency_item, parent, false)
        )
    }

    override fun onBindViewHolder(holder: SecondCurrencyRecyclerAdapter.ViewHolder, position: Int) {
